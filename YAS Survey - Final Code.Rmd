...
#Extracting a statistic for number of respondents in original dataset to use later
count_original <- Wyoming_YAS_2022 %>%
  nrow()
...
####Missing Race
```{r}
# 1. Create race variable list
race_vars <- c("demo_2___a", "demo_2___b", "demo_2___c", "demo_2___d",
               "demo_2___e", "demo_2___f", "demo_2___g", "demo_2___h")

# 2. Ensure race variables are numeric
Wyoming_YAS_2022_edit <- Wyoming_YAS_2022 %>%
  mutate(across(all_of(race_vars), ~ as.numeric(.)))

# 3. Add a binary column for missing race selections
Wyoming_YAS_2022_edit <- Wyoming_YAS_2022_edit %>%
  mutate(missing_race = ifelse(rowSums(select(., all_of(race_vars)), na.rm = TRUE) == 0, 1, 0))

# 4. Exclude observations where missing_race == 1 & wt > 0
Wyoming_YAS_2022_filtered <- Wyoming_YAS_2022_edit %>%
  filter(!(missing_race == 1 & wt > 0))

rm(Wyoming_YAS_2022_edit)
```

4531 in original dataset, 4352 after excluding no-race respondents. Difference of 179
...
num_missing <- Wyoming_YAS_2022_filtered %>%
...
If I were to exclude all of the people who did not answer all of the variables used in the fact sheets that would exclude 366 respondents.
...
excluded_respondents <- Wyoming_YAS_2022_filtered %>%
  filter(rowSums(is.na(select(., all_of(mandatory_vars)))) > 0)

# Calculate the number of respondents who were excluded
total_excluded <- nrow(excluded_respondents)

# Initialize a vector to hold the percentage of answered variables for each excluded respondent
answered_percentages <- numeric(total_excluded)

# Loop through each excluded respondent to calculate their answered percentage
for (i in 1:total_excluded) {
  answered_count <- sum(!is.na(excluded_respondents[i, mandatory_vars]))
  answered_percentages[i] <- (answered_count / length(mandatory_vars)) * 100
}
...
# Loop through percentage increments of 10%
for (j in seq(0, 100, by = 10)) {
  range_count <- sum(answered_percentages >= j & answered_percentages < (j + 10))
  percentage_distribution <- rbind(percentage_distribution, data.frame(percentage_range = paste(j, "-", j + 9, "%", sep = ""), count = range_count))
}
...
eligible_respondents <- Wyoming_YAS_2022_filtered %>%
...
tobacco_check <- eligible_respondents %>%
  mutate(across(all_of(tobacco_vars), ~ as.numeric(.)))

# Identify respondents in eligible_respondents who answered at least one of the secondary variables
tobacco_check <- tobacco_check %>%
  mutate(missing_tobacco = ifelse(rowSums(select(., all_of(tobacco_vars)), na.rm = TRUE) == 0, 1, 0))

# Filter the dataset and count the number of rows where missing_tobacco == 1
count_missing_tobacco <- tobacco_check %>%
  filter(missing_tobacco == 1) %>%
  nrow()

# 4. Exclude observations where missing_race == 1 & wt > 0
final_exclusions <- tobacco_check %>%
  filter(!(missing_tobacco == 1 & wt > 0))


print(paste("Number of respondents excluded who didn't answer at least 1 tobacco question:",  count_missing_tobacco))

count_final <- final_exclusions %>%
  nrow()

print(paste("Percentage of original respondents left:", round((count_final / count_original) * 100, 1), "%"))
print(paste("Number of respondents in final dataset:", count_final))

rm(tobacco_check, eligible_respondents)
```
...
# Creating a variable for the number of tobacco products individuals smoked
final_exclusions$tobacco_all <- rowSums(final_exclusions[, c("t___a", "t___b",  "t___c", "t___d",  "t___e", "t___f")], 
                                            na.rm = TRUE)
# Check the number of rows before mutation
num_rows_before <- nrow(final_exclusions)

# Creating a yes/no tobacco variable
YAS_clean <- final_exclusions %>%
...
sum(is.na(final_exclusions$tobacco_all))
...
YAS_design <- svydesign(data=YAS_clean, weights=~wt, id=~1)
...
county <- subset(YAS_clean, county == 23)
...
```{r}
#Current alcohol use [a] - defined as drank alcohol at least one day in the past month

#Statewide distribution of Responses by alcohol use [a]
...
# Convert 'a_2' to a factor with all desired levels
prop_a2.cty$a_2 <- factor(prop_a2.cty$a_2, levels = c(0, 1, 2, 3, 4, 5))

# Recode the factor levels into meaningful labels
prop_a2.cty$varname <- recode(prop_a2.cty$a_2, 
                              '0' = "None", 
                              '1' = "1 day", 
                              '2' = "2 days", 
                              '3' = "3-5 days", 
...
# Create a summary table for County data that includes all levels and the rest of the columns
prop_a2.cty <- prop_a2.cty %>%
  group_by(varname, .drop = FALSE) %>%
  dplyr::summarize(across(where(is.numeric), \(x) sum(x, na.rm = TRUE)),  # Using anonymous function for summing
            first_a_2 = first(a_2),  # Ensure 'a_2' is explicitly renamed
            first_group = first(group)) %>%  # Ensure 'group' is also renamed
  mutate(across(where(is.numeric), ~replace(., is.na(.), 0))) %>%
  dplyr::rename(a_2 = first_a_2,         # Rename first_a_2 to a_2
                group = first_group)     # Rename first_group to group
...
#Combining Statewide and County
prop_ta.cty$varname <- recode(prop_ta.cty$t___a, '1' = "Yes",
                          '0' = "No")
...
YAS_clean$tobacco_all <- rowSums(YAS_clean[,c("t___a", "t___b",
                                                            "t___c", "t___d", 
                                                            "t___e", "t___f")])

num_tobacco_products <-  YAS_clean %>%
  dplyr::count(tobacco_all)
...
YAS_clean$race_count <- rowSums(YAS_clean[,c("demo_2___a",
                                                            "demo_2___b",
                                                            "demo_2___c",
                                                            "demo_2___d", 
                                                            "demo_2___e",
...
prop_demo4$varname <- dplyr::recode(prop_demo4$demo_4, '1' = "Less than a high school degree", 
...
#Combining Statewide and County
prop_demo4.cty$varname <- dplyr::recode(prop_demo4.cty$demo_4,  '1' = "Less than a high school degree", 
                        '2' = 'High School graduate/GED',
                        '3' = "Some college, no degree",
                        '4' = "Occ, tech or vocational program",
                        '5' = "College degree",
...
# Recode the tobacco_yn and mh variables for the county-level data
...
# Filter for "Nearly every day" and "More than half days" and create a new category
